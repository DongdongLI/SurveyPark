...
<html>
	<head>
		<title>Engage in Survey</title>
		<link rel="stylesheet" type="text/css" href='<s:url value="/styles.css" />'>
	</head>
	<body>
		<s:include value="/header.jsp" />
		<s:if test="surveys.isEmpty()">There're no available surveys!</s:if>
		<s:else>
			...
			<table>
				...
				<tr>
					<td colspan='<s:property value="#cells"/>' class="tdHeader">Please select a survey</td>
				</tr>
				<tr>
					<td colspan='<s:property value="#cells"/>' style="height: 5px"></td>
				</tr>
				<tr>
					<td class="tdListHeader" colspan='<s:property value="#cells"/>'>Please select a survey</td>
				</tr>
						...
						<s:iterator var="j" begin="0" end="%{#cells-1}" step="1">
							...
							<td width='<s:property value="100/#cells" />%'>
								<s:if test="#idx < surveys.size">
									<s:a action="engageSurvey_entry?sid=%{surveys[#idx].id}" cssClass="aList" namespace="/">
